...
def build_post_html(
    ...
    The hero_image_url is rendered as a full-width featured image block
    right at the top of the post content (WordPress also sets it as the
    post thumbnail via the REST API featured_media field separately).
    """
...
<div class="pp-testi">
  ...
  <p class="pp-testi-author"><strong>{t["name"]}</strong> <span>{t["role"]}</span></p>
    ...
    # Featured image is set via WordPress featured_media API (post thumbnail).
    # The theme renders it above the post title automatically — NOT inline here.
    featured_img_html = ""  # intentionally empty — no duplicate
...
.pp-wrap h2 {{
  ...
  margin: 44px 0 16px;
  padding-bottom: 10px;
  border-bottom: 3px solid #1dbb84;
  line-height: 1.3;
}}
.pp-wrap h3 {{
  font-family: "Public Sans", system-ui, sans-serif;
...
.pp-wrap li {{ margin-bottom: 8px; color: #777777; }}

/* ── Disclosure (tiny footnote at article end) ────────── */
.pp-disclosure {{
  font-family: 'Public Sans', system-ui, sans-serif;
  font-size: 11px;
  color: #bbb;
...
/* ── CTA Button ───────────────────────────────────────── */
@keyframes pp-pulse {{
  0%, 100% {{ box-shadow: 0 6px 20px rgba(29,187,132,0.4), 0 0 0 0 rgba(29,187,132,0.3); }}
  50% {{ box-shadow: 0 6px 20px rgba(29,187,132,0.6), 0 0 0 12px rgba(29,187,132,0); }}
}}
.pp-cta-btn {{
  display: block;
  width: 100%;
  max-width: 520px;
  margin: 0 auto;
  background: linear-gradient(135deg, #1dbb84 0%, #17a372 100%);
  color: #fff !important;
  text-align: center;
...
.pp-cta-btn:hover {{
  ...
  box-shadow: 0 8px 28px rgba(29,187,132,0.5);
  color: #fff !important;
  text-decoration: none !important;
}}
.pp-cta-micro {{
...
.pp-cta-micro span {{ color: #1dbb84; font-weight: 700; }}
...
.pp-cta-box {{
  background: linear-gradient(135deg, #f0fdf8 0%, #f9fafb 100%);
  border: 2px solid #1dbb84;
  border-radius: 12px;
  padding: 32px 28px;
  margin: 40px 0;
  text-align: center;
...
.pp-pricing-card .pp-pc-label {{
  display: inline-block;
  background: linear-gradient(135deg, #1dbb84, #17a372);
  color: #fff;
  font-family: 'Public Sans', system-ui, sans-serif;
  font-size: 11px;
  font-weight: 800;
...
.pp-testi {{
  ...
  padding: 20px 22px;
  margin-bottom: 16px;
  border-left: 4px solid #1dbb84;
  box-shadow: 0 2px 8px rgba(0,0,0,0.05);
}}
.pp-testi:last-child {{ margin-bottom: 0; }}
.pp-testi-stars {{ color: #1dbb84; font-size: 16px; margin-bottom: 8px; }}
...
<div class="pp-wrap">
  ...
  <!-- TINY DISCLOSURE FOOTNOTE -->
  <p class="pp-disclosure">This page contains affiliate links. We may earn a small commission if you purchase through our links, at no extra cost to you.</p>
...
def create_and_publish_landing_page(
    ...
    landing_html = build_post_html(
        ...
        # If hero came from Excel (not scraped), include images[0] as product image too
